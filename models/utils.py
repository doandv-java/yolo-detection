...
from models.configs import YOLO_CLASSES


def load_class_names(class_name_file):
    names = {}
    with open(class_name_file, 'r') as data:
...
def load_yolo_weights(model, weights_file):
    ...
    if configs.YOLO_MODEL == "v3":
        range1 = 75
        range2 = [58, 66, 74]
    if configs.YOLO_MODEL == "v4":
        range1 = 110
        range2 = [93, 101, 109]
...
def draw_bbox(image, bboxes, classes=configs.YOLO_CLASSES, show_label=True, show_confidence=True,
              Text_colors=(255, 255, 0), rectangle_colors='', tracking=False):
    NUM_CLASS = load_class_names(classes)
...
# detect from webcam
def detect_realtime(net_model, output_path, input_size=416, show=True, classes=configs.YOLO_CLASSES,
                    score_threshold=0.3,
    ...
    while True:
        ...
        t1 = time.time()
        pred_bbox = net_model.predict(image_frame)
        t2 = time.time()
...
def detect_video(yolo, video_path, output_path, input_size=416, show=False, classes=YOLO_CLASSES,
